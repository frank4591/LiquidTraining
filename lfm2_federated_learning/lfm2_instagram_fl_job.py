...
def main():
    args = define_parser()
    train_script = "src/hf_lfm2_instagram_fl.py"
    ...
    # Persistor configuration commented out to ensure training works
    # The training script handles model loading, training, and saving checkpoints internally
    # This approach works perfectly for federated learning without external model management complexity
    ...
    for i in range(num_clients):
        ...
        script_args = f"--model_name_or_path {model_name_or_path} --data_path_train {data_path} --data_path_valid {data_path} --output_path {output_path} --train_mode {train_mode} --message_mode {message_mode} --clean_up {clean_up}"
